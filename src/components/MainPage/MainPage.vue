  ...
  export default defineComponent({
    ...
    methods: {
      ...
      async getExcursionByName(){
        ...
        const copy = this.allExcursionArr.filter((el) => {
          if (isCityMatches(el) && el.title.toLowerCase().includes(this.searchName.toLowerCase())) {
            return true
          }
        });
  ...
  <div class="container">
    ...
    <div class="inputs">
      <div class="nameSearch">
        <i class="fa-times"></i>
        <input class="search" placeholder="Введите название экскурсии" v-model="this.searchName" @click='this.searchName = ``' v-on:keyup="getExcursionByName()" >
      ...
      <div class="dropdown-wrapper">
        <div  class="dropdown-popover" >
          <div>
            <input placeholder="Выбрать город" v-model="this.selectedCity.name">
            <FontAwesomeIcon @click="isVisible = !isVisible" class="input-icon" :icon="faChevronDown()" :style="{ color: '#999999' }"/>
          </div>
    ...
    <div class="cards" v-if="this.excursionToDisplayArr.length !== 0 && (this.isNameEmpty() || this.isCityEmpty())">
      <div class="card" v-for="(excursion, index) in excursionToDisplayArr" :key="{index}">
